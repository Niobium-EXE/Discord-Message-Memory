(() => {
  ...
  function parseLocalMessage(element, context) {
    ...
    const record = {
      ...
      links: [...element.querySelectorAll(":scope > .content a[href], .content a[href]")].map(anchor => ({ url: anchor.getAttribute("href") || "", text: (anchor.textContent || "").trim() })).filter(item => item.url),
  ...
  function parseMessageMemoryDomRecord(element, channelId, channelMeta, exportedAt) {
    ...
    const record = {
      ...
      links: [...element.querySelectorAll(".message-links a[href]")].map(anchor => ({ url: anchor.getAttribute("href") || "", text: (anchor.textContent || "").trim() })).filter(item => item.url),
